...
class JavMenuWeb(object):
    ...
    _webparsers = {
        "jav_list": {
            ...
            "item": {
                ...
                "is_downloadable": ['.//span[@class="badge bg-primary text-white d-none d-md-inline-block"]/text()', False],
                "has_subtitle": ['.//span[@class="badge bg-secondary text-white d-none d-md-inline-block"]/text()', False],
            },
            "format": {
                "id": lambda x : x.replace(" ",""),
                "title": lambda x : "" if not x else x.replace("\n", "").strip(),
                "is_downloadable": lambda x : True if x == "可下载" else False,
                "has_subtitle": lambda x : True if x == "字幕" else False,
            },
            "filter": {
        ...
        "detail_info": {
            "id": ['//*[@id="app"]/div/div[2]/div[2]/div/div[1]/div[4]/h1/strong/text()', lambda x : is_jav(x)],
            "title": '//*[@id="app"]/div/div[2]/div[2]/div/div[1]/div[4]/h1/strong/text()',
            'img': '//*[@id="player"]/@data-poster',
            "date": ['//*[@id="app"]/div/div[2]/div[2]/div/div[1]/div[5]/div/div/div[3]/span[2]/text()', lambda x:None if not x else x.strip()],
            "videoLength": ['//*[@id="app"]/div/div[2]/div[2]/div/div[1]/div[5]/div/div/div[4]/span[2]/text()', lambda x:None if not x else x.replace('分钟','').strip()],
        }
    }
    ...
    def rank_list(cls, type="censored", rank_type="day", page=1):
        ...
        for ranking, jav in enumerate(jav_list):
            jav['ranking'] = ranking + 1
